...
function App() {
  ...
  const [errors, setErrors] = useState<Array<{ type?: string; [k: string]: unknown }>>([])
  const [lastValidationMessage, setLastValidationMessage] = useState<string>('Not yet validated')
  const [isValidating, setIsValidating] = useState(false)

  useEffect(() => {
    const loadAssets = async () => {
      const [pretextRnc, pretextRng, pretextDevRnc, testGoodXml] = await Promise.all([
      ...
      setSchemaAssets({ pretextRnc, pretextRng, pretextDevRnc, testGoodXml })
      setXmlText(testGoodXml)
      setLastValidationMessage('Assets loaded. Click Validate to run schema checks.')
    }

    loadAssets().catch((e) => {
      const message = e instanceof Error ? e.message : String(e)
      setLastValidationMessage(`Failed to load symlinked test assets: ${message}`)
    })
  }, [])
  ...
  const runValidation = () => {
    ...
    try {
      ...
      if (result.errors.length === 0) {
        ...
        setLastValidationMessage(`Found ${result.errors.length} schema error(s).`)
      }
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e)
      setErrors([])
    ...
    <main className="page">
      <header className="topbar">
        <div>
          ...
          <p>Live XML editing on the left, schema validation diagnostics on the right.</p>
        </div>
        <div className="controls">
          ...
          <select
            id="schema-preset"
            value={schemaPreset}
            onChange={(e) => setSchemaPreset(e.target.value as SchemaPreset)}
          >
            <option value="rnc">{SCHEMA_LABELS.rnc}</option>
            <option value="rng">{SCHEMA_LABELS.rng}</option>
            <option value="dev-plus-rnc">{SCHEMA_LABELS['dev-plus-rnc']}</option>
          </select>
          <button type="button" onClick={runValidation} disabled={isValidating}>
      ...
      <PanelGroup direction="horizontal" className="workspace-panel-group">
        <Panel defaultSize={55} minSize={25} className="workspace-panel">
          <section className="pane pane-editor">
            <h2>XML Editor</h2>
            <p className="hint">Loaded from symlinked tests/test-good.xml; edit and validate.</p>
            <CodeMirror
              value={xmlText}
              ...
              <ol className="errors">
                {errors.map((err, idx) => (
                  <li key={`${err.type ?? 'unknown'}-${idx}`}>
                    <div className="error-type">{err.type ?? 'UnknownError'}</div>
